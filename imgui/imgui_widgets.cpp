...
bool ImGui::rav_button(ImVec2 min, ImVec2 max, ImColor base_color, const char* name) {
    const ImGuiID id = ImGui::GetID(name);
    bool hovered, held;

    ImRect rect = ImRect(min, max);
    ItemAdd(ImRect(min, max), id);
    bool pressed = ButtonBehavior(rect, id, &hovered, &held);
    //RenderFrame(rect.Max, rect.Min, base_color);

    ImVec2 text_size = CalcTextSize(name);
    RenderText((rect.Max + rect.Min) / 2 - text_size / 2, name);

    return pressed;
}

bool ImGui::tab_vidvigaemyi(ImVec2& position, bool* is_open, int* offset_from_tab, int* current_tab)
{
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    ImDrawList* draw_list = window->DrawList;
    const ImGuiID id = ImGui::GetID("TABLISSSSSSSSSSST");
    const ImGuiIO IO = ImGui::GetIO();

    static std::map<ImGuiID, float> animation_map;
    auto animation = animation_map.find(id);
    if (animation == animation_map.end()) {
        animation_map.insert( { id, 30 } );
        animation = animation_map.find(id);
    }

    bool hovered, out_held;
    ImColor color = ImColor(0.f, 0.f, 0.f, 0.f);

    if (*is_open)
        animation->second = ImLerp(animation->second, 120.f, IO.DeltaTime * 7.f);
    else
        animation->second = ImLerp(animation->second, 30.f, IO.DeltaTime * 7.f);

    ImRect rect = ImRect(position, ImVec2(position.x + 30, window->Size.y * 12));
    bool pressed = ButtonBehavior(rect, id, &hovered, &out_held);
    ItemAdd(rect, id);
    ImRect rect_to_draw = ImRect(position, ImVec2(position.x + animation->second, window->Size.y * 12));
    RenderFrame(rect_to_draw.Min, rect_to_draw.Max, IM_COL32_BLACK, true);
    if (pressed)
        *is_open = !*is_open;
    *offset_from_tab = animation->second;

    if (*is_open) {
        ImRect button_rect = ImRect(rect_to_draw.Min + ImVec2(5, 30), rect_to_draw.Min);
        if (rav_button(button_rect.Min, button_rect.Min + ImVec2(animation->second, 30), IM_COL32_WHITE, "Player"))
            *current_tab = 1;

        button_rect.Min.y += 35;
        if (rav_button(button_rect.Min, button_rect.Min + ImVec2(animation->second, 30), IM_COL32_WHITE, "Aimbot"))
            *current_tab = 2;
        button_rect.Min.y += 35;
        if (rav_button(button_rect.Min, button_rect.Min + ImVec2(animation->second, 30), IM_COL32_WHITE, "Map"))
            *current_tab = 3;
        button_rect.Min.y += 35;
        if (rav_button(button_rect.Min, button_rect.Min + ImVec2(animation->second, 30), IM_COL32_WHITE, "Movement"))
            *current_tab = 4;
        button_rect.Min.y += 35;                    
        if (rav_button(button_rect.Min, button_rect.Min + ImVec2(animation->second, 30), IM_COL32_WHITE, "Misc"))
            *current_tab = 5;
        button_rect.Min.y += 35;
        if (rav_button(button_rect.Min, button_rect.Min + ImVec2(animation->second, 30), IM_COL32_WHITE, "Dev"))
            *current_tab = 6;
    }
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    auto animation = animation_map.find(id);
    if (animation == animation_map.end()) {
        animation_map.insert({ id, 0 });
        animation = animation_map.find(id);
    }

    if (*v)
        animation->second = ImLerp<float>(animation->second, 1, IO.DeltaTime * 9);
    else
        animation->second = ImLerp<float>(animation->second, 0, IO.DeltaTime * 9);
    ...
    if (is_visible)
    {
        ...
        ImColor check_col = ImColor(13.f, 0.f, 31.f, animation->second);
